...
class ARC2_StoreEndpoint extends ARC2_Store {
  ...
  function handleRequest($auto_setup = 0) {
    if (!$this->isSetUp()) {
      if ($auto_setup) {
        ...
        $this->result = 'Missing configuration or the endpoint store was not set up yet. '.MSG__need2setup_endpoint;
      }
    }
  ...
  //DAF 
  function go($auto_setup = 0,$params=array()) {
    $this->handleRequest($auto_setup,$params);
    $this->sendHeaders();
    echo $this->getResult();
  }
  ...
  function getHTMLDocHead($params=array()) {
    return '
    	<head>
  ...
  function getHTMLDocForm() {
    $q = $this->p('query') ? htmlspecialchars($this->p('query')) : "PREFIX skos: <http://www.w3.org/2004/02/skos/core#> \n SELECT * WHERE {\n  GRAPH ?g { ?s ?p ?o . }\n}\nLIMIT 10";
    return '
    ...
    return '
      <div class="options">
        ...
        <dl>
          ...
          <dd>
            <input type="checkbox" id="show_inline" name="show_inline" value="1" ' . ($this->p('show_inline') ? ' checked="checked"' : '') . ' />
          </dd>
